...
watch_t dispLCD_timeWatch = {TIME_WATCH, 12, 0, 0, 0};
watch_t dispLCD_stopWatch = {STOP_WATCH, 0, 0, 0, 0};
...
static void Presenter_DispLCDMonitor();
...
void Presenter_lcdOutTimeWatchData(watch_t watchData){
	memcpy(&dispLCD_timeWatch, &watchData, sizeof(watch_t));
}
void Presenter_lcdOutStopWatchData(watch_t watchData){
	memcpy(&dispLCD_stopWatch, &watchData, sizeof(watch_t));
}

void Presenter_DispLCDMonitor(){
	Presenter_DispLCDTimeWatch(dispLCD_timeWatch);
	Presenter_DispLCDStopWatch(dispLCD_stopWatch);
}
