...
FND_TypeDef fndDigitCom[4] = { // common 위치
      {GPIOA, GPIO_PIN_12},
      {GPIOC, GPIO_PIN_5},
      {GPIOC, GPIO_PIN_6},
      {GPIOC, GPIO_PIN_8}
...
void FND_WriteDp(uint8_t dpData, uint8_t dpState)
{
   if (dpState == FND_DP_ON)
      fndDispDp |= dpData;   // 해당 위치 값 1로 만들어줌
   else
      fndDispDp &= ~dpData;   // 해당 위치 값 0로 만들어줌
}
...
void FND_DispDigit(uint16_t digit)
{
   const uint8_t segFont[12] = {
         ...
         0x7F, // 8 -> 01111111
         0x6F, // 9 -> 01101111
         0x80, // 10(dpin) -> 10000000
         0x00  // 11(dpoff) -> 00000000
   };
