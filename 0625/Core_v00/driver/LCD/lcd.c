...
void LCD_cmdMode() // command 모드이면 RS = 0
{
   lcdData &= ~(1<<LCD_RS);
   LCD_sendI2C(lcdData);
}

void LCD_charMode() // data 모드이면 RS = 1
{
   lcdData |= (1<<LCD_RS);
   LCD_sendI2C(lcdData);
}

void LCD_writeMode() // Write 이면 RW = 1
{
   lcdData &= ~(1<<LCD_RW);
   LCD_sendI2C(lcdData);
}
...
void LCD_writeCmdData(uint8_t data) // 내부 명령어 실행
{
   LCD_writeMode();
   LCD_cmdMode();
   LCD_sendData(data);
...
void LCD_writeCharData(uint8_t data) // LCD 한 글자 찍기
{
   LCD_writeMode();
   LCD_charMode();
   LCD_sendData(data);
