...
void StopWatch_dotCallBack(){
	if((stopWatch.msec == 500-1) || (stopWatch.msec == 1000-1)){
		stopWatch.dp[3] = !stopWatch.dp[3];
	}
	if ((stopWatch.msec %50-1) ==0){
		stopWatch.dp[1] = !stopWatch.dp[1];
	}
}
...
void StopWatch_Stop()
{
   ...
   FND_WriteData((stopWatch.min % 10 *1000) + (stopWatch.sec*10) + (stopWatch.msec/100),stopWatch.dp);
}
